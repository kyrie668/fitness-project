...
export default function TrainingPlanGrid() {
  ...
  const totalPages = useMemo(() => {
    if (curCategory !== '全部') {
      return Math.ceil(fitnessData.filter((item) => item.category === curCategory).length / 10);
    }
    return Math.ceil(fitnessData.length / 10);
  }, [fitnessData, curCategory]);

  useEffect(() => {
    setCurData(fitnessData.slice((currentPage - 1) * 10, currentPage * 10));
